...
The phase-by-phase implementation log for this project. See the
[main README](../README.md) for the current-state summary — this document
is the detailed build history behind it: what each phase actually built,
what was verified live against real external APIs vs. covered by
deterministic tests, and specific findings and trade-offs made along the
way.
...
- [x] Phase 11 — operator dashboard expansion (six tabs, business-wide activity feed) + test-database isolation fix
...
`pytest` itself never calls the real API — all 47 tests (at the time)
used `FakeInterpreter` or small purpose-built stubs.
...
At the time this phase was originally built, it had not yet been
exercised against the real Resend API. That changed during Phase 11's
final demo pass: a real booking confirmation email was sent through the
live Resend API and confirmed delivered via the Resend dashboard —
see the main README's [Status](../README.md#status) table.
`ResendNotificationProvider` accepts an optional `httpx` transport for
exactly this reason: tests inject an `httpx.MockTransport` to verify the
success and failure paths without a real network call, and a real
deployment supplies real credentials via `.env`
(`RESEND_API_KEY`, `RESEND_FROM_EMAIL`) without any code changes.
...
non-sensitive), `POST /interpret` (rate-limited but not API-key gated),
and `POST /jobs/run-tick` (system-wide, not business-scoped) stay
unauthenticated -- known, documented gaps this phase didn't extend the
per-business model to cover (see the main README's
[Known limitations](../README.md#known-limitations)). This was also the
first HTTP-level test coverage `app/api/routes.py` has ever had.
...
Phase 11 detail: the operator dashboard grew from one escalation-queue
page into a six-tab view (Overview, Appointments, Customers,
Notifications, Jobs, Escalations) exposing data the system already
persisted but had no UI for. Seven new read-only endpoints
(`backend/app/api/routes_dashboard.py`) assemble responses entirely from
existing tables (`AuditEvent`, `Notification`, `WorkflowStep`,
`ScheduledJob`) -- no workflow or state-machine changes, no new business
logic. The appointment timeline endpoint is the most involved piece: it
merges `WorkflowStep`s across every `ProcessingRun` that ever touched one
appointment (original booking, reschedule, no-show recovery) with
`AuditEvent`s and `Notification`s, sorted chronologically -- proven
against a real no-show → recovery → rebooking chain. A new business-wide
`/v1/activity` endpoint powers the Overview tab's "what has the
automation been doing" feed, distinct from the existing per-appointment
`/trace` endpoint.

This phase also surfaced and fixed a real incident: running the full test
suite against the same database the local demo stack was using deleted
all of that session's demo data via `pytest`'s autouse table-cleanup
fixture. The fix — deriving a genuinely separate test database and
overriding FastAPI's `get_db` dependency for the whole test session — is
documented in the main README's [Testing](../README.md#testing) section,
along with the permanent regression test it added
